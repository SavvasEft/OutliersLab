...
if show_z_stats: 
    if data_dimensions == 1:
        ...
        fig = get_z_score_iter_outlier_plots(data = np_data, z_threshold=z_threshold_ax0,line_plot=show_line_plot)
        st.pyplot(fig)
        ...
        with col1:
            ...
            z_threshold_ax0 = st.slider('Choose threshold for z-scores for ax1:', min_value=1., \
                                    max_value = 10., step = 0.25, value = 3.)
            fig1 = get_z_score_iter_outlier_plots(data = np_data[0], z_threshold=z_threshold_ax0, line_plot=show_line_plot)
            st.pyplot(fig1)
        with col2:
            
            st.write('Analyzing axes 1:')
            z_threshold_ax1 = st.slider('Choose threshold for z-scores for ax2:', min_value=1., \
                                    max_value = 10., step = 0.25, value = 3.)
            fig2 = get_z_score_iter_outlier_plots(data = np_data[1], z_threshold=z_threshold_ax1, line_plot=show_line_plot)
            st.pyplot(fig2)
